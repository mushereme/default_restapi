...
exports.login = asyncHandler(async (req, res, next) => {
    ...
    if(user.status === "9") {
        ...
        const shareholders = await req.db.shareholder.findAll({
            attributes: ['id', 'lastname', 'firstname', 'register', 'amount', 'voted', 'userId', 'companyId', 'createdAt', 'updatedAt'],
            where: {
        ...
        if(shareholders.length === 0) {
            throw new MyError('Регистрын дугаарт харгалзах тархалтын мэдээлэл олдсонгүй', 400)
        }
...
exports.register = asyncHandler(async (req, res, next) => {
    ...
    if(!req.body.register) {
        throw new MyError('Та регистрын дугаараа илгээнэ үү.', 400)
    }
    ...
    try {
        for(let i = 0; i < shareholders.length; i++) {
            if(shareholders[i].userId) {
                throw new MyError(`${shareholders[i].register} тархалтын мэдээлэлд хэрэглэгч бүртгүүлсэн байна.`, 400)
            } 
        }
    } catch (e) {
        throw new MyError(e, 400)
