...
const sendSMS = async ({phone, messageMn, messageEn}) => {
    ...
    if(unitel_prefix.includes(prefix)) {
        ...
        url = `http://203.91.114.131/cgi-bin/sendsms?username=${process.env.gmobile_username}&password=${process.env.gmobile_password}&from=${process.env.gmobile_number}&to=${phone}&text=${messageMn}`
    ...
    await axios.get(fixed_url)
        ...
        .catch((err) => {
            throw new MyError('Уг SMS илгээхэд алдаа гарлаа.' + err, 400)
        })
}   
